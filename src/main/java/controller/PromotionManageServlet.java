...
public class PromotionManageServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        List<String> statuses = pd.getStatuses();
        List<Promotion> list = pd.getAllPromotion();
        session.setAttribute("statuses", statuses);
        request.setAttribute("ListPromotion",list );
        request.getRequestDispatcher("/view/promotion-list.jsp").forward(request, response);
    } 
