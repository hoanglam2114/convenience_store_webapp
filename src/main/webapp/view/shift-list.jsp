                    ...
                    <div class="content-wrapper">
                        <div class="container-xxl flex-grow-1 container-p-y">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <div class="page-title">Quản lý ca làm</div>
                            </div>
                                ...
                                <div class="row mb-3 align-items-end">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select id="weekday" name="weekday" class="form-control">
                                            ...
                                            <option value="Monday" ${param.weekday == 'Monday' ? 'selected' : ''}>Thứ Hai</option>
                                            <option value="Tuesday" ${param.weekday == 'Tuesday' ? 'selected' : ''}>Thứ Ba</option>
                                            <option value="Wednesday" ${param.weekday == 'Wednesday' ? 'selected' : ''}>Thứ Tư</option>
                                            <option value="Thursday" ${param.weekday == 'Thursday' ? 'selected' : ''}>Thứ Năm</option>
                                            <option value="Friday" ${param.weekday == 'Friday' ? 'selected' : ''}>Thứ Sáu</option>
                                            <option value="Saturday" ${param.weekday == 'Saturday' ? 'selected' : ''}>Thứ Bảy</option>
                                            <option value="Sunday" ${param.weekday == 'Sunday' ? 'selected' : ''}>Chủ Nhật</option>
                                        </select>
                                    </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <c:forEach var="day" items="${shift.workingDays}">
                                                            <div>
                                                                <c:choose>
                                                                    <c:when test="${day == 'Monday'}">Thứ Hai</c:when>
                                                                    <c:when test="${day == 'Tuesday'}">Thứ Ba</c:when>
                                                                    <c:when test="${day == 'Wednesday'}">Thứ Tư</c:when>
                                                                    <c:when test="${day == 'Thursday'}">Thứ Năm</c:when>
                                                                    <c:when test="${day == 'Friday'}">Thứ Sáu</c:when>
                                                                    <c:when test="${day == 'Saturday'}">Thứ Bảy</c:when>
                                                                    <c:when test="${day == 'Sunday'}">Chủ Nhật</c:when>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="deleteShift?id=${shift.shiftId}"
                                                           ...
                                                           onclick="return confirm('Bạn chắc chắn muốn xóa ca này?');" title="Xóa">
                            ...
                            <div class="container-xxl d-flex justify-content-center py-3">
                                <ul class="pagination pagination-rounded mb-0">
                                    <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                        <a class="page-link"
                                           href="ListShift?index=${currentPage - 1}&keyword=${param.keyword}&fromTime=${param.fromTime}&toTime=${param.toTime}&weekday=${param.weekday}">&laquo;</a>
                                    </li>
                                    <c:forEach begin="1" end="${endPage}" var="i">
                                        <li class="page-item ${i == currentPage ? 'active' : ''}">
                                            <a class="page-link"
                                               href="ListShift?index=${i}&keyword=${param.keyword}&fromTime=${param.fromTime}&toTime=${param.toTime}&weekday=${param.weekday}">${i}</a>
                                        </li>
                                    </c:forEach>
                                    <li class="page-item ${currentPage == endPage ? 'disabled' : ''}">
                                        <a class="page-link"
                                           href="ListShift?index=${currentPage + 1}&keyword=${param.keyword}&fromTime=${param.fromTime}&toTime=${param.toTime}&weekday=${param.weekday}">&raquo;</a>
                                    </li>
                                </ul>
                            </div>
                        </footer>
